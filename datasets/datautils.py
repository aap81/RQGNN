...
def split_graph_dataset(graphs, datasets_path, dataset, train_ratio, test_ratio):
    ...
    train_path = os.path.join(dataset, "raw", dataset + TRAIN)
    val_path = os.path.join(dataset, "raw", dataset + VAL)
    test_path = os.path.join(dataset, "raw", dataset + TEST)

    np.savetxt(train_path, train_index, fmt='%d')
    np.savetxt(val_path, val_index, fmt='%d')
    np.savetxt(test_path, test_index, fmt='%d')
    np.savetxt(os.path.join(dataset, "raw", dataset + NEWLABEL), graphlabels, fmt='%d')